...
cViewer::cViewer() 
{
	m_windowName = L"Project Cars Motion Viewer";
...
bool cViewer::OnInit()
{
	...
	if (!m_shmMem.Init("$pcars$", sizeof(SharedMemory)))
		return false;
...
void cViewer::OnUpdate(const float elapseT)
{
	...
	SharedMemory *shm = (SharedMemory*)m_shmMem.m_memPtr;
	if (!shm)
		return;

	const float roll = shm->mOrientation[VEC_Z];
	const float pitch = shm->mOrientation[VEC_X];
	const float yaw = shm->mOrientation[VEC_Y];
	D3DXMatrixRotationYawPitchRoll((D3DXMATRIX*)&m_carTm, yaw, -pitch, -roll);
}
